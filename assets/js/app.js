...
function isAvailable(productId, start, end) {
  ...
  // 1️⃣ BLOKADY DNI TYGODNIA Z PANELU ADMINA
  for (const b of adminBlocks) {
    if (b.type === "weekday" && b.weekday) {
      if (b.product === "all" || b.product === productId) {
        // sprawdzamy każdy dzień w zakresie
        for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
          const name = dayNames[d.getDay()];
          if (name === b.weekday) {
            return false; // jeśli którykolwiek dzień jest np. Sunday → niedostępne
          }
        }
      }
    }
  ...
  // 2️⃣ BLOKADY ZAKRESÓW DAT Z PANELU ADMINA
  for (const b of adminBlocks) {
  ...
  // 3️⃣ LOKALNE BLOKADY (availability)
